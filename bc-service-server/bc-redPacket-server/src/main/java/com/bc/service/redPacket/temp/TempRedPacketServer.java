...
@Service
@Slf4j
public class TempRedPacketServer {
    @Autowired
    ...
    @Autowired
    private TempRobotServer tempRobotServer;
    @Autowired
    ...
    public ResponseResult playRedPacket(RedPacketDto redPacketDto) throws Exception{
        HongBaoYuTime hongBaoYuTime = hongBaoServer.getHongBaoYuTime();
        if (hongBaoYuTime.getCountDown() > 0) {
            return ResponseResult.FAIL("活动未开始");
        }
    ...
    public ResponseResult userPlay(VsAwardPlayer player,RedPacketDto redPacketDto) throws Exception{
        ...
        //执行
        tempRobotServer.pay(record,player);
